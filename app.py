...
def calculate_swing_score_and_bands(df):
    if df is None or len(df) < 20: return 0, 0, 0, "계산불가", 0, 0, False
    try:
        df = df.copy()
        ...
        # ⚠️ 거래량 필터 로직 추가
        is_volume_spike = volume_now > volume_avg * 2.0
        ...
        return int(score), current, rsi, buy_min, buy_max, ma20, is_volume_spike
    except:
        return 40, 0, 50.0, 0, 0, 0, False
...
def calculate_kr_realtime_score(code):
    ...
    try:
        ...
        score, _, rsi, buy_min, buy_max, ma20, _ = calculate_swing_score_and_bands(df)
...
def fetch_us(stock):
    try:
        ...
        score, current, rsi, buy_min, buy_max, ma20, _ = calculate_swing_score_and_bands(df)
...
def fetch_crypto(coin):
    try:
        ...
        score, current, rsi, buy_min, buy_max, ma20, is_volume_spike = calculate_swing_score_and_bands(df)
        # ⚠️ 거래량 폭발 필터 (평균 거래량 대비 2배 이상인 놈만 통과)
        if not is_volume_spike or current == 0: return None
...
def get_portfolio_market_data(name):
    ...
    try:
        ...
        if not df.empty and len(df) >= 5:
            s, c, r, b_min, b_max, ma20, _ = calculate_swing_score_and_bands(df)
...
try: coins_list = pyupbit.get_tickers(fiat="KRW")
...
for title, data, sym in [("🇺🇸 해외 알짜 성장주 TOP 3", us_top, "$"), ("🪙 거래량 폭발 코인 TOP 3", crypto_top, ""), ("🔥 국내 테마/거래대금 대장주 TOP 3", kr_top, "₩")]:
    st.header(title)
    if not data:
...
if st.session_state.my_portfolio:
    ...
    for i, p in enumerate(st.session_state.my_portfolio):
        ...
        if curr == 0:
            ...
            if st.button(f"❌ {name} 강제 삭제", key=f"err_del_{i}"):
                to_remove = i
            continue
